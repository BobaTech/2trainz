...
for yr in range(2011, 2015): # only thse years... dammit
    ...
    acc_all_data[str(yr)] = [[
        float(row[list(acc_raw.columns.values).index("latitude")]),
        float(row[list(acc_raw.columns.values).index("longitud")]),
        float(row[list(acc_raw.columns.values).index("accdmg")])
    ...
    cas_all_data[str(yr)] = [[
        float(row[list(cas_raw.columns.values).index("latitude")]),
        float(row[list(cas_raw.columns.values).index("longitud")]),
        True if row[list(cas_raw.columns.values).index("casfatal")] == "Y" else False
